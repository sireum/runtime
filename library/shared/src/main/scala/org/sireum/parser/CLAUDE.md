...
Used by `NGrammar`, `PredictiveTable`, and `LexerDfas` to embed serialized data in generated parser files.

- **`toCompact`**: `MessagePack.writer(T)` → write fields (`w.writeZ`, `w.writeU32`, `w.writeString`, `w.writeB`, etc.) → `ops.StringOps.toBase64(ops.ISZOps.lz4Compress(w.result))`. Follows standard [MessagePack](https://msgpack.org) binary format.
- **`fromCompact`**: `ops.ISZOps.lz4Decompress(ops.StringOps.fromBase64(s).left).left` → `MessagePack.reader(data)` → `r.init()` → `r.readZ`/`r.readU32`/etc.
- **`toCompactST`**: convenience returning `st"""Xxx.fromCompact("${toCompact}")"""`
- For `@sig` hierarchies: write an integer tag via `w.writeZ(tag)` before fields; read it back to dispatch to the correct constructor.
- **Auto-generated**: `sireum tools sergen` generates `JSON`/`MsgPack` objects for non-generic, non-nested-collection `@datatype`/`@record`/`@sig`/`@msig` types (e.g., `slang/tipe/shared/.../JSON.scala`). JSON serializer adds a `type` field (class simple name); keys must be in field-declaration order.
